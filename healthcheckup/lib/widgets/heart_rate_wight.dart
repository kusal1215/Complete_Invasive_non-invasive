...
import 'package:camera/camera.dart';
import 'package:flutter/material.dart';
import 'package:healthcheckup/components/chart.dart';
import 'package:healthcheckup/providers/non_invasive_provider.dart';
import 'package:healthcheckup/widgets/scan_animation.dart';
import 'package:provider/src/provider.dart';
import 'package:wakelock/wakelock.dart';

class HeartRateWidgt extends StatefulWidget {
  const HeartRateWidgt({Key key}) : super(key: key);

  @override
  _HeartRateWidgtState createState() => _HeartRateWidgtState();
}

class _HeartRateWidgtState extends State<HeartRateWidgt>
  ...
  void _updateBPM() async {
    ...
    while (_toggled) {
      ...
      if (_counter > 0) {
        ...
        print(_bpm);
        setState(() {
          int bpm = ((1 - _alpha) * this._bpm + _alpha * _bpm).toInt();
          if (bpm > 30 && bpm < 150) {
            values.add(bpm);

            if (values.length == 5) {
              averageTotal = average_1();
              _untoggle();
              context.read<NonInvasiveProvider>().addHeartRate(averageTotal);
            }
          }
          this._bpm = bpm;
        });
